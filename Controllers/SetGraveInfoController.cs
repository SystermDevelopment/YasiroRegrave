...
using static YasiroRegrave.Pages.ReienEditModel;
...
namespace YasiroRegrave.Controllers
{
    ...
    public class SetGraveInfoController : Controller
    {
        ...
        public IActionResult SetGraveInfo([FromBody] GraveInfoRequest request)
        {
            ...
            foreach (var info in request.Data)
            {
                ...
                if(reienCode != "")
                {
                    var existingReien = _context.Reiens.Where(r => r.DeleteFlag == 0 && r.ReienCode == reienCode).FirstOrDefault();
                    ...
                    string cemetery = parts[2];

                    // エリア
                    if (area != "")
                    {
                        var existingAria = _context.Areas.Where(r => r.DeleteFlag == 0 && r.AreaCode == area).FirstOrDefault();
                        if (existingAria == null)
                        {
                            // INSERT
                            var newArea = new Area
                            {
                                ReienIndex = reienIndex,
                                AreaCode = area,
                                AreaName = area, //TODO:本来のNameをもらったら修正すること
                            ...
                            existingAria = newArea;
                        }
                        else
                        {
                            // UPDATE
                            existingAria.AreaName = area; //TODO:本来のNameをもらったら修正すること
                            existingAria.UpdateDate = DateTime.UtcNow;
                            existingAria.UpdateUser = null;
                            existingAria.DeleteFlag = 0;
                            _context.SaveChanges();
                        }
                        // 区画
                        if (section != "")
                        {
                            var existingSection = _context.Sections.Where(r => r.DeleteFlag == 0 && r.SectionCode == section).FirstOrDefault();
                            if (existingSection == null)
                            {
                                // INSERT
                                var newSection = new Section
                                {
                                    AreaIndex = existingAria.AreaIndex,
                                    SectionCode = section,
                            ...
                            if (cemetery != null)
                            {
                                var existingCemetery = _context.Cemeteries.Where(r => r.DeleteFlag == 0 && r.CemeteryCode == cemetery).FirstOrDefault();
    ...
    {
        public List<GraveInfo> Data { get; set; }
    }
